...
tableFreq=as.data.frame(ToPlot) #converting the table with numbers into the data frame and called it Table of Freq
...
base= ggplot(data = tableFreq, 
             aes(x = locale, #aes - aestatics; choose the columns which to plot
                 y = pct))
...
plot1 = base + geom_bar(fill ="gray",
                        stat = 'identity') # y is just what it is! plotting what it sees, no calculations are done by R
plot1
```
...
base + geom_bar(stat = 'identity') #identity means that we use the value
```
...
h1= h1 + stat_bin(binwidth = 20, aes(label=..count..), 
                  geom = "text",boundary = 0,vjust=-0.5) #putting numbers of values in each bin abouve bins
h1
```
...
hist(eduwa3$Reduced.Lunch,labels = T,xlab="Reduced Lunch", xaxt="n") #T - TRUE  
...
eduwa3$redLunchOrd=cut(eduwa3$Reduced.Lunch,
                       ...
                       include.lowest = T, #T - TRUE
                       ordered_result = T) #T - TRUE
```
...
freq(eduwa3$redLunchOrd,style = 'rmarkdown',report.nas = F) #F - FALSE
```
...
# strategy 2: 
eduwa4=eduwa[complete.cases(eduwa$Student.Teacher.Ratio),] #complete.cases means keep the complete values of this variable and then shrink the dataset ==> we have a new dataset without missing variables 
...
#The county witht he min average Student/teacher ration has 11.30
```
...
TitleText4='Density of Student-Teacher Ratio' 
